...
class GameSimulationTest {
    ...
    fun `advance day consumes colony supplies and produces ore`() {
        val initial = DemoGame.initialState()

        val result = GameSimulation.advanceDay(initial)
    ...
    fun `power shortage reduces ore output`() {
        val initial = DemoGame.initialState().copy(
            colony = DemoGame.initialState().colony.copy(
                powerAvailable = 10,
                powerDemand = 24,
            ),
        )
    ...
    fun `day 365 advances to a new year`() {
        val initial = DemoGame.initialState().copy(year = 3, day = 365)
